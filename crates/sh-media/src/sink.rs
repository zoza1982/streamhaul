//! Frame sink abstractions for the client presentation pipeline.
//!
//! A [`FrameSink`] is the consumer end of the decode pipeline: the pipeline calls
//! [`FrameSink::deliver`] for each decoded [`VideoFrame`] and the sink is free to
//! display, buffer, or discard the frame as appropriate.
...
mod tests {
    use super::*;
    use crate::{PixelFormat, Resolution};
    use bytes::Bytes;
