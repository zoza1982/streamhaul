...
| P4-3 | TURN client (RFC 8656): Allocate/Refresh/CreatePermission/ChannelBind codec + long-term credential auth + SimTurnServer + `latency-probe relay steering` (LLD §4.3); coturn deploy **deferred** → R-COTURN-DEPLOY | sh-ice | P4-2 | network-engineer | 401-challenge + channel roundtrip + relay-steering tests (53 ✅) | 🟡 | |
...
**Gate P4:** 🟡 P2P when possible (NAT-sim matrix: FullCone×*, RestrictedCone×RestrictedCone, PortRestricted×PortRestricted → P2P nominated; Symmetric×Symmetric → relay fallback nominated — all verified in `agent::tests::nat_matrix`, P4-2 ✅) · 🟡 connects across symmetric NAT via relay (TURN client codec + SimTurnServer + 401-challenge sequence + ChannelBind verified in `turn::tests::*`, P4-3 🟡; live coturn server wiring deferred → R-COTURN-DEPLOY) · ☐ relay carries only opaque ciphertext (zero-knowledge verified — P4-5).
