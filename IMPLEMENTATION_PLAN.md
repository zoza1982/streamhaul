...
**Gate P4:** 🟡 P2P when possible (NAT-sim matrix: FullCone×*, RestrictedCone×RestrictedCone, PortRestricted×PortRestricted → P2P nominated; Symmetric×Symmetric → relay fallback nominated — all verified in `agent::tests::nat_matrix`, P4-2 ✅) · 🟡 connects across symmetric NAT via relay (relay candidate model + steering scoring implemented; live coturn / TURN Allocate deferred to P4-3) · ☐ relay carries only opaque ciphertext (zero-knowledge verified — P4-5).
...
| R-ICE-LIVE-NAT | **Live STUN/TURN server communication and real NAT traversal are deferred (P4-2).** `sh-ice` (P4-2) delivers the complete STUN codec, ICE state machine, NAT simulator, relay scoring, and TURN-HMAC credential generator — all verified hermetically against the in-process `NatSimNetwork`. What is deferred to P4-3 (coturn deploy + infra session): (a) TURN Allocate / Refresh / CreatePermission / ChannelBind message sequences; (b) live coturn deployment and the coturn REST credential endpoint integration; (c) live NAT traversal on real internet paths (requires a running coturn server and two peers behind real NAT boxes); (d) `IceAgent::gather()` issuing real STUN Binding Requests to live servers to discover srflx addresses (currently host-only). The `UdpTransport` trait, `TurnCredentials::generate`, and the relay scoring API (`score_relays`, `select_relay`) are designed for drop-in connection to a live coturn instance. **Do not use `sh-ice` for production ICE until P4-3 wires in live STUN/TURN; the current agent produces host candidates only.** | P4-3 | network-engineer, devops-engineer |
| R-ICE-FUZZ | **Schedule a nightly fuzz job for `stun_decode` (sh-ice/fuzz).** The `stun_decode` libFuzzer target exercises all STUN decode paths against hostile bytes. The fuzz crate compiles (verified with `cargo +nightly build`) but no nightly CI fuzz job exists yet (tracked in X-2). Fuzz continuously in an automated job; any panic that emerges from the fuzz corpus is a security regression. | X-2 / GA | qa-engineer |
